    ...
    <UContainer>
      ...
      <header class="mb-10 text-center animate-in fade-in duration-500">
        <div class="flex justify-center mb-4">
          <div class="w-16 h-16 rounded-2xl bg-primary-500 flex items-center justify-center shadow-lg shadow-primary-500/20 text-white">
            <UIcon name="i-lucide-bot" class="w-10 h-10" />
          </div>
        </div>
        <h1 class="text-4xl font-black tracking-tight text-gray-900 dark:text-white">{{ siteTitle }}</h1>
        <p class="text-gray-500 mt-2 italic">{{ siteSubtitle }}</p>
      </header>

      <!-- Stats Grid -->
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 mb-10">
        <UCard v-for="item in statusItems" :key="item.label" class="shadow-sm border-none bg-white dark:bg-gray-900">
          <div class="flex items-center gap-4">
            <div class="p-3 rounded-xl" :class="`bg-${item.color}-500/10 text-${item.color}-500`">
              <UIcon :name="item.icon" class="w-6 h-6" />
            </div>
            <div>
              <div class="text-xs text-gray-500 font-bold uppercase tracking-wider">{{ item.label }}</div>
              <div class="text-2xl font-black text-gray-900 dark:text-white">{{ item.value }}</div>
            </div>
          </div>
        </UCard>
      </div>
      ...
      <div class="grid grid-cols-1 gap-8">
        ...
        <section class="space-y-6 max-w-2xl mx-auto w-full">
          <h2 class="text-xl font-bold flex items-center gap-2 text-gray-900 dark:text-white">
            <UIcon name="i-lucide-network" class="text-gray-400" />
            {{ t('active_endpoints') }}
          </h2>
          
          <div class="space-y-4">
            <UCard variant="subtle" class="border border-gray-100 dark:border-gray-800">
              <template #header>
                <div class="flex justify-between items-center py-0.5">
                  <div class="flex items-center gap-2">
                    <UIcon name="i-simple-icons-openai" class="text-[#00A67E]" />
                    <span class="font-bold text-gray-900 dark:text-white">OpenAI V1</span>
                  </div>
                  <UButton icon="i-lucide-copy" size="xs" variant="ghost" color="neutral" @click="copyToClipboard(`${baseUrl}/api/openai/v1`)" />
                </div>
              </template>
              <div class="space-y-2">
                ...
                <div v-if="settings?.showApiKeyPublic && settings?.enableApiKeyAuth" class="text-[10px] text-amber-500 uppercase font-bold tracking-widest flex items-center gap-1">
            ...
            <UCard variant="subtle" class="border border-gray-100 dark:border-gray-800">
              <template #header>
                <div class="flex justify-between items-center py-0.5">
                  <div class="flex items-center gap-2">
                    <UIcon name="i-simple-icons-anthropic" class="text-[#D97757]" />
                    <span class="font-bold text-gray-900 dark:text-white">Claude V1</span>
                  </div>
                  <UButton icon="i-lucide-copy" size="xs" variant="ghost" color="neutral" @click="copyToClipboard(`${baseUrl}/api/claude`)" />
                </div>
              </template>
              <div class="space-y-2">
      ...
      <footer class="mt-20 pt-10 border-t border-gray-200 dark:border-gray-800 flex justify-between items-center text-xs text-gray-400">
        <div class="flex items-center gap-4">
            ...
            <UButton to="/agent" variant="link" color="neutral" size="xs" icon="i-lucide-lock">{{ t('admin_dashboard') }}</UButton>
            <UButton to="https://github.com/huangdihd/call_me_as_agent" target="_blank" variant="link" color="neutral" size="xs" icon="i-simple-icons-github">GitHub</UButton>
        </div>
        <div>v1.0.0</div>
      </footer>
    </UContainer>
